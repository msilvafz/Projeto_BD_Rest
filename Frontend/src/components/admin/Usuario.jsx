...
const Usuario = () => {
    ...
    const [editedUsuario, setEditedUsuario] = useState({ nome: '', email: '', senha: '', tipo: '' });
    const [newUsuario, setNewUsuario] = useState({ nome: '', email: '', senha: '', tipo: 'Aluno' });
    ...
    const fetchUsuarios = async () => {
        try {
            const responseAlunos = await axios.get('http://localhost:3000/user/tipo/Aluno');
            const responseProfessores = await axios.get('http://localhost:3000/user/tipo/Professor');
            const responseAdmins = await axios.get('http://localhost:3000/user/tipo/Admin');
    ...
    const handleSave = async () => {
        try {
            const response = await axios.put(`http://localhost:3000/user/${editingId}`, editedUsuario);
            const updatedUsuario = response.data;
            setUsuarios(usuarios.map(usuario => usuario._id === editingId ? updatedUsuario : usuario));
            setEditingId(null);
    ...
    const handleAdd = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:3000/user', newUsuario);
            const addedUsuario = response.data;
            setUsuarios(prevUsuarios => [...prevUsuarios, addedUsuario]);
            setNewUsuario({ nome: '', email: '', senha: '', tipo: 'Aluno' });
    ...
    const renderUserTable = (tipo) => (
        <Table striped bordered hover style={{ textAlign: 'center' }}>
            ...
            <tbody>
                {usuarios.filter(usuario => usuario.tipo === tipo).map((usuario) => (
                    <tr key={usuario._id}>
                        <td>{usuario._id}</td>
                        <td>
